...
import { 
  ...
  MoreVertical, PackageSearch, Trash2, Eye
} from 'lucide-react';
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#0A1D11] transform transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-full flex flex-col p-6">
          <div className="flex items-center gap-3 mb-10">
            <div className="bg-lime-400 p-1.5 rounded-lg shadow-lg shadow-lime-400/20"><Leaf className="w-6 h-6 text-[#0A1D11]" /></div>
                         ...
                         <tr key={u.id} className="hover:bg-neutral-50 transition-colors">
                            ...
                            <td className="px-8 py-6">
                               <span className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-widest ${
                                 u.user_type === 'Farmer' ? 'bg-blue-100 text-blue-700' : 
          ...
          {activeTab === 'Products' && (
            <div className="space-y-8 animate-in fade-in duration-500">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {products.length === 0 && (
                  <div className="md:col-span-3 py-20 bg-white rounded-[3rem] border-2 border-dashed border-neutral-100 flex flex-col items-center justify-center text-neutral-400">
